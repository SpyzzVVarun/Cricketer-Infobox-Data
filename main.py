...
for page_title in titles:
    try:
        if(os.path.exists(f'infoboxes/{page_title}')):
            continue
        json_dict = dict()
        global_lst = []
        championship_years = {}
        ...
        columnAddendum(json_dict)
        clubAddendum(json_dict)
        ...
        with open(f"infoboxes/{page_title}.json", "w", encoding="utf-8") as f:
            new_dict = dict()
